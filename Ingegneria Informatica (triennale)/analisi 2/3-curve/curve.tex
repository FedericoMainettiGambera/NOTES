\section{Funzioni $\mathbb{R} \rightarrow \mathbb{R}^n$ ("Funzioni di una variabile a valori vettoriali", "Curve nel piano e nello spazio")}
...
\newline
Vediamo il caso in cui si vuole calcolare la \textbf{lunghezza di una curva in forma cartesiana (una funzione\dots)} $y=f(x)$:\newline
...
\]
\ \newline
Vediamo il caso in cui su vuole calcolare la \textbf{lunghezza di una curva in forma polare} $\rho= f(\theta)$:\newline
Sia $\gamma$ una curva piana regolare in forma polare, ossia:
\[
    \gamma : \begin{cases}
        x = f(\theta) cos(\theta) \\
        y= f(\theta) sin(\theta)
...
allora
\[
    l(\gamma) = \int_{\theta_1}^{\theta_2}\sqrt{g(\theta)^2 + g'(\theta)^2}d \theta
\]
...
\newline
Se $\vec{r} = \vec{r}(s)$ è un curva parametrizzata mediante il parametro arco, il vettore derivato $\vec{r}'(s)$ coincide col \textbf{versore tangente} $\vec{T}$.
\newline
\newline
...
\begin{itemize}
    ...
    \[
        ...
        \end{cases}
    \]
    dove $\rho$ è la densità lineare e $m$ è la massa (calcolabile come $\int_{\gamma}\rho ds$).\newline
...
\end{cases}$. Ora per scrivere l'equazione della retta tangente in un certo punto $\vec{r}(t_0) = \left[\begin{matrix}
    x(t_0)\\ y(t_0) \\ z(t_0)
\end{matrix}\right]$ è sufficiente calcolare il valore di $\vec{r}' (t)$ in $t = t_0$ e scrivere:
\[
...
\[
    ...
    \end{cases}
\]
\subsubsection{Interpretazione geometrica degli integrali di prima specie}
Data una linea $\gamma$ nel piano $(x,y)$ e data una funzione $z = f(x,y)$ (una superficie) definita e non negativa in corrispondenza dei punti di $\gamma$, l'integrale
\[
    \int_{\gamma} f(x,y) ds 
\]
può essere visto come l'area della superficie verticale compresa tra la linea $\gamma$ nel piano $(x,y)$ e la superficie $z = f(x,y)$
\subsubsection{Integrali di linea di seconda specie}
Come per gli integrali di prima specie, anche per gli integrali di seconda specie le variabiliche compaiono  nell'integrale vengono trasformate nel parametro della linea $\gamma$; mentre nel caso degli integrali di prima specie $ds$ diventava $|\vec{r}'(t)|dt$, ora negli integrali compaiono i termini $dx, dy, dz$ che diventano rispettivamente
\[
    dx = x'(t) dt \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;dy = y'(t) dt \;\;\;\;\;\;\;\;\;\;\;\;\;\;\;dz = z'(t) dt
\]
Da notare è che negli integrali di seconda specie, siccome le variabili di integrazione sono $x,y,z$, il risultato è legatoal verso di percorrenza della linea.